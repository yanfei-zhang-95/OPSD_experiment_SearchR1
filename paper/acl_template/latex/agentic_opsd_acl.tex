...
The final version should summarize one currently available model checkpoint and its corresponding validation or test performance. Ideally this includes exact match, average number of turns, valid-action ratio, valid-search count, and any currently logged OPSD statistics.
...
\toprule
Model & EM & Turns & Valid Act. & Valid Search & Notes \\
\midrule
Baseline & \textit{--} & \textit{--} & \textit{--} & \textit{--} & \textit{to be filled} \\
\method{} & \textit{--} & \textit{--} & \textit{--} & \textit{--} & \textit{current run} \\
\bottomrule
\end{tabular}
\caption{Current training result placeholder.}
